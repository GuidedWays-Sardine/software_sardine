...
Item{
    ...
    // Propriétés liées à la position et à la taille du INI_floatinput
    property double default_x: 0              // Position du INI_floatinput pour les dimensions minimales de la fenêtre (w_min*h_min)
    property double default_y: 0
    property double default_width: 100        // Dimensions du INI_floatinput pour les dimensions minimales de la fenêtre (w_min*h_min)
    ...
    // Propriétés liées à l'état du INI_floatinput
    ...
    // Fonction pour remettre la valeur par défaut dans le INI_floatinput (maximum_value si is_max_default sinon minimum_value)
    function clear(){
        ...
        var changed = root.is_max_default ? root.visible_value !== validator.bottom : root.visible_value !== validator.top
        body.text = ""
    ...
    function change_unit(new_unit) {
        ...
        if(root.conversion_list.length > 1 && new_unit != unit_text.text) {
            ...
            // Cas où l'unité n'a pas été trouvée, laisse un message de registre
                ...
                console.log(`L'unité \"${new_unit}\" pour le INI_floatinput : \"${root.objectName}\" inexistante. \n ${root.conversion_list}`)
            }
        }
    }

    // Fonction de clignotement des bordures (met le INI_floatinput en valeur)
    function blink(time=3, period=0.5, color=root.yellow) {
        ...
        if(time > 0 && period > 0) {
            ...
            if(!regex_color.test(color)) {
                console.log(`Valeur hex : \"${color}\" pour le clignotement du INI_floatinput : \"${root.objectName}\" invalide. Clignotement en jaune.`)
                color = root.yellow
            }
        ...
        // Sinon, la période ou le temps de clignotement est négatif, l'indique
            ...
            console.log(`La période et le temps de clignotement du INI_floatinput : \"${root.objectName}\" ne peuvent pas être négatif (temps : ${time}s ; période : ${period}s)`)
        }
    }
    ...
    // Détecte quand la valeur minimale est changée
    onMinimum_valueChanged: {
        ...
        if(validator.bottom > validator.top){
            ...
            console.log(`Nouvelle limite inférieure pour le INI_floatinput : \"${root.objectName}\" trop grande (min : ${validator.bottom} > ${validator.top} : max)`)
    ...
    // Détecte quand la valeur maximale est changée
    onMaximum_valueChanged: {
        // Cas où la valeur maximale est inférieure à la valeur minimale
        if(validator.top < validator.bottom){
            // Change la borne minimale pour être identique à la nouvelle borne maximale
            console.log(`Nouvelle limite supérieure pour le INI_floatinput : \"${root.objectName}\" trop faible (max : ${validator.top} < ${validator.bottom} : min)`)
    ...
    TextField {
        ...
        // Indique que seul des nombres peuvent être entrés
        ...
        MouseArea
        {
            ...
            // Détecte lorsque le INI_floatinput est cliqué, permet de démarrer l'édition de la valeur
            onClicked: {
