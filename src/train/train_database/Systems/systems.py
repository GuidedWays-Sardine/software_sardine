...
class Systems:
    ...
    def __init__(self, train_settings):
        ...
        self.braking = braking.Braking(train_settings)

        # Si le train a été paramétré de façon complexe, récupère les données sinon génère un train complex
        if train_settings.get_value("mode", "Mode.SIMPLE").lower() == 'mode.complex':
            self.read_train(train_settings)
        else:
            self.generate(train_settings)

    def read_train(self, train_settings):
        ...
        ------
        ValueError
            Jetée lorsque l'un des éléments du train n'est pas en quantité suffisante
        """
        ...
        index = 0
        try:
            while f"railcar{index}.mission_type" in train_settings:
                self.frame.add_railcar((tdb.Mission[train_settings[f"railcar{index}.mission_type"][8:]],
                                        tdb.Position[train_settings[f"railcar{index}.position_type"][9:]],
                                        train_settings[f"railcar{index}.position_type"],
                                        train_settings[f"railcar{index}.levels"], train_settings[f"railcar{index}.doors"],
                                        train_settings[f"railcar{index}.Mtare"], train_settings[f"railcar{index}.Mfull"],
                                        train_settings[f"railcar{index}.length"],
                                        [train_settings[f"railcar{index}.{leter}empty"] for leter in ["A", "B", "C"]],
                                        train_settings[f"railcar{index}.multiply_mass_empty"],
                                        [train_settings[f"railcar{index}.{leter}full"] for leter in ["A", "B", "C"]],
                                        train_settings[f"railcar{index}.multiply_mass_full"]))
                index += 1
        ...
        if index < 1:
            raise ValueError("Un train ne peut pas avoir aucune voiture paramétrée.")

        # Continue avec les bogies et leurs systèmes de freinage
        index = 0
        try:
            while f"bogie{index}.linked_railcars" in train_settings:
                self.traction.add_bogie((None if train_settings[f"bogie{index}.position_type"] is None else tdb.Position[train_settings[f"bogie{index}.position_type"][9:]]
                                         ,
                                         train_settings[f"bogie{index}.linked_railcars"],
                                         train_settings[f"bogie{index}.axles_count"],
                                         None, train_settings[f"bogie{index}.axles_power"]))

                # Récupère les paramètres de chacun des systèmes de freinages et ajoute le nombre de systèmes nécessaire
                brakes_parameters = [None if not train_settings[f"bogie{index}.pad.count"] else
                                     (),  # TODO récupérer tous les paramètres du freinage par plaquette
                                     None if not train_settings[f"bogie{index}.disk.count"] else
                                     (),  # TODO récupérer tous les paramètres du freinage par disque
                                     None if not train_settings[f"bogie{index}.magnetic.count"] else
                                     (),  # TODO récupérer tous les paramètres du freinage par patins magnétiques
                                     None if not train_settings[f"bogie{index}.foucault.count"] else
                ...
                self.braking.modify_brakes_count(self.traction.bogies[-1],
                                                 [train_settings[f"bogie{index}.{brake}.count"] for brake in ["pad", "disk", "magnetic", "foucault"]],
                                                 brakes_parameters)
                index += 1
        except KeyError as error:
    ...
    def generate(self, train_settings):
        ...
        average_bogies_count = train_settings["bogies_count"] / train_settings["railcars_count"]
        ...
        average_brakes = [int(train_settings[f"{b_s}_brakes_count"] / train_settings["bogies_count"]) for b_s in ["pad", "disk", "magnetic", "foucault"]]
        ...
        if average_bogies_count <= 2:
            front_bonus_bogies = int(((train_settings["bogies_count"] - (train_settings["railcars_count"] + 1)) + 1) / 2.0)
            back_bonus_bogies = int((train_settings["bogies_count"] - (train_settings["railcars_count"] + 1)) / 2.0)
        else:
            front_bonus_bogies = int((train_settings["bogies_count"] - int(average_bogies_count) * train_settings["railcars_count"] + 1) / 2.0)
            back_bonus_bogies = int((train_settings["bogies_count"] - int(average_bogies_count) * train_settings["railcars_count"]) / 2.0)

        # Vérifie que le train n'a pas un taux de motorisation > à 100% (minimum entre essieux moteurs et essieux)
        motorized_count = min(train_settings["motorized_axles_count"], train_settings["bogies_count"] * train_settings["axles_per_bogie"])
        ...
        motorized_axle_weight = train_settings["motorized_axle_weight"]
        carrying_axle_weight = (train_settings["weight"] - motorized_axle_weight * train_settings["motorized_axles_count"]) / \
                               (train_settings["bogies_count"] * train_settings["axles_per_bogie"] - train_settings["motorized_axles_count"])
        ...
        for car_index in range(train_settings["railcars_count"]):
            ...
            next_articulated = average_bogies_count < 2 and (front_bonus_bogies <= car_index < train_settings["railcars_count"] - back_bonus_bogies - 1)

            # Détermine le nombre d'essieux centraux à ajoutés (valable que si average_bogies_count > 2)
            middle_bogies_count = (int(average_bogies_count - 2) + (front_bonus_bogies > car_index or car_index > (train_settings["railcars_count"] - back_bonus_bogies - 1))
            ...
            if (previous_bogies_count * train_settings["axles_per_bogie"]) < front_motorized_axles:
                new_motorized_count += min(front_motorized_axles - previous_bogies_count  * train_settings["axles_per_bogie"],
                                           new_bogies_count * train_settings["axles_per_bogie"])
            if (next_bogies_count * train_settings["axles_per_bogie"]) < back_motorized_axles:
                new_motorized_count += min(back_motorized_axles - next_bogies_count * train_settings["axles_per_bogie"],
                                           new_bogies_count * train_settings["axles_per_bogie"] - new_motorized_count)
            ...
            front_axles_motorized_count = min(new_motorized_count, train_settings["axles_per_bogie"] * (not previous_articulated))
            # Motorisation prioritaire sur les essieux avant et arrière
            back_axles_motorized_count = min(new_motorized_count - front_axles_motorized_count, train_settings["axles_per_bogie"])
            ...
            middle_axles_motorized_count = [max(min(remain_motorized - c_m_c * train_settings["axles_per_bogie"], train_settings["axles_per_bogie"]), 0)
            ...
            if previous_articulated:
                ...
                railcar_weight = (previous_bogie.get_motorized_axles_count() * motorized_axle_weight +
                                  (train_settings["axles_per_bogie"] - previous_bogie.get_motorized_axles_count()) * carrying_axle_weight) / 2
            else:
                ...
                railcar_weight = (front_axles_motorized_count * motorized_axle_weight +
                                  (train_settings["axles_per_bogie"] - front_axles_motorized_count) * carrying_axle_weight)
            ...
            for c_m_a_c in middle_axles_motorized_count:
                ...
                railcar_weight += (c_m_a_c * motorized_axle_weight +
                                   (train_settings["axles_per_bogie"] - c_m_a_c) * carrying_axle_weight)
            ...
            railcar_weight += ((back_axles_motorized_count * motorized_axle_weight +
                                (train_settings["axles_per_bogie"] - back_axles_motorized_count) * carrying_axle_weight) /
                               (1 + next_articulated))
            ...
            front_brakes_count = ([0, 0, 0, 0]
                                  ...
                                  [average_brakes[0] + (int((train_settings["bogies_count"] - bonus_brakes[0])/2) <=
                                                        previous_bogies_count <
                                                        int(train_settings["bogies_count"] - (train_settings["bogies_count"] - bonus_brakes[0])/2)),
                                   average_brakes[1] + (int((train_settings["bogies_count"] - bonus_brakes[1]) / 2) <=
                                                        previous_bogies_count <
                                                        int(train_settings["bogies_count"] - (train_settings["bogies_count"] - bonus_brakes[1]) / 2)),
                                   average_brakes[2] + (bonus_brakes[2] > previous_bogies_count),
                                   average_brakes[3] + (train_settings["bogies_count"] - (bonus_brakes[3]) <= previous_bogies_count)])

            middle_brakes_count = []
            for c_b in range(middle_bogies_count):
                middle_brakes_count.append([average_brakes[0] + (int((train_settings["bogies_count"] - bonus_brakes[0])/2) <=
                                            ...
                                            average_brakes[3] + (train_settings["bogies_count"] - (bonus_brakes[3]) <= (previous_bogies_count + (not previous_articulated) + c_b))])

            back_brakes_count = ([average_brakes[0] + (int((train_settings["bogies_count"] - bonus_brakes[0])/2) <=
                                  ...
                                  average_brakes[3] + (train_settings["bogies_count"] - (bonus_brakes[3]) <= (previous_bogies_count + (not previous_articulated) + middle_bogies_count))])
            ...
            abc = [round(train_settings[abc_i] * (0.85 if not car_index else 0.15 / (train_settings["railcars_count"] - 1)), 5) for abc_i in ["a", "b", "c"]]
            ...
            if not previous_articulated:
                self.traction.add_bogie(traction.Bogie(position_type=tdb.Position.FRONT,
                                                       ...
                                                       axles_count=train_settings["axles_per_bogie"],
                                                       motorized_axles=front_axles_motorized_count,
                                                       axles_power=train_settings["axles_power"]))
            ...
            self.frame.add_railcar(frame.Railcar(mission_type=general_mission,
                                                 position_type=(tdb.Position.FRONT if car_index == 0 else
                                                                tdb.Position.BACK if car_index == (train_settings["railcars_count"] - 1)
                                                 ...
                                                 length=train_settings["length"] / train_settings["railcars_count"],
                                                 ABCempty=abc,
    ...
    def get_settings(self):
        ...
        parameters.update(self.frame.get_settings())
        parameters.update(self.traction.get_settings(self.braking))   # S'occupe des systèmes de traction et de freinage
